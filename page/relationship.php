...
// echo $collage.$collage_profile_id.$date.$month.$year.$date_2.$month_2.$year_2.$experience.$branch.$degree;
// exit(0);
//
if(isset($_GET['edit'])){
$edit = mysqli_real_escape_string($connection,$_GET['edit']);
    $query = "INSERT INTO `about`
(`id`, `user`, `about_code`, `share_with`, `connect_privacy`, `add_profile_name`, `branch`, `add_profile`, `position`, `my_opinion`, `start_date`, `start_month`, `start_year`, `end_date`, `end_month`, `end_year`)
VALUES (NULL, '{$_SESSION['id']}', '6', '1', NULL, {$degree}, {$branch}, {$collage_profile_id}, {$collage}, {$experience}, {$date}, {$month}, {$year}, {$date_2}, {$month_2}, {$year_2});";
    $query = "UPDATE `about` SET 
    `position` = {$collage},
    `branch` =   {$branch},
    `add_profile` =   {$collage_profile_id},
    `add_profile_name` =   {$degree},
...
VALUES (NULL, '{$_SESSION['id']}', '4', '1', NULL, {$collage}, {$branch}, {$collage_profile_id}, {$degree}, {$experience}, {$date}, {$month}, {$year}, {$date_2}, {$month_2}, {$year_2});";
}
        ...
        <div class="main-signup " id="signup-form">
            ...
            <form class="multi-stage" autocomplete="off" id="form" method="post" enctype="multipart/form-data">
                <div class="forms" autocomplete="off" spellcheck="false">
                    ...
                    <span class="note note-plus">What ia your relationship status?</span>
                    <div class="input-wrap" >
                        <select class="fields" name="collage" id="select_relation" required="" >
                            <option value="" selected=""></option>
                            <?php 
                            
//                            $saved_relationship_status
                                  $all_echo  = array("Single","Committed","Married","Divorced");

                            foreach($all_echo as $value){
                                if($value == $saved_relationship_status){
                        ...
                        <span class="label select">Status</span>
                    </div>
                    <input type="hidden" id="collage_profile_id" name="collage_profile_id" value="<?php echo $saved_id;?>">

                    <div class="button-wrap">
                        <div data-id="1" class="continue" id="first_page_continue">Continue</div>
                    </div>
                </div>
                ...
                <div class="forms" autocomplete="off" spellcheck="false">
                    ...
                    <span class="note note-plus">With whom? (Optional)</span>
                    <div class="input-wrap">
                        <input type="text" class="fields" id="location" name="degree" value="<?php echo $saved_name;?>" required>
                ...
                <div class="forms" autocomplete="off" spellcheck="false" id="signup_username">
                    ...
                    <span class="note note-plus">When did you commit this relationship?</span>
                    <div class="date-wrap">
                        <div class="input-wrap">
                    ...
                    <span class="note note-plus  "><input type="checkbox" id="i_work_here" onchange="show_ending_date()" checked> I am still in this relationship. </span>
                    <div class="hide hide_a"><span class="note note-plus  ">I feel sorry for you but would you like to share when it got over? </span></div>
                ...
                <div class="forms" autocomplete="off" spellcheck="false">
                    ...
                    <span class="note note-plus">How do you see your relationship life? (Optional)</span>
                    <div class="input-wrap">
 ...
 <input type="hidden" name="submitted" value="1">
             ...
             <?php 
            if(isset($_GET['edit'])){
echo '<a href="?delete='.$_GET['edit'].'" id="skipall" >Delete relationship</a>';
            }
            ?>
        </div>
