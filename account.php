            ...
            xhttp.onreadystatechange = function () {
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    for (var i = 0; i < output.post.length; i++) {
                        ...
                        my_adds = '<div class="card card_own_' + output.post[i].owner_id + '" post_id="' + output.post[i].id + '">'+shared_with+' <div class="follow-conn"> <img src="' + post_own_pic + '" class="follow-icon users" oncontextmenu="this.requestFullscreen();this.setAttribute('+"'"+'src'+"'"+','+'this.getAttribute('+"'"+'src'+"'"+').replace('+"'"+'profile/i/120/'+"'"+', '+"'"+'profile/i/1080/'+"'"+')'+');return false;"/> <div class="conn-name"> <span> <b>' + output.post[i].name + ' <small class="u_c_' + output.post[i].owner_id + '">' + tagshow + '</small> <small class="fllw_' + output.post[i].owner_id + '" onclick="unfollow_ys(' + output.post[i].owner_id + ',' + output.post[i].account_type + ')">'+follow_integ+'</small><small class="unmt_' + output.post[i].owner_id + '" onclick="mute_ys(' + output.post[i].owner_id + ')">'+mute_integ+'</small> </b> </span> <span>' + output.post[i].introduction + '</span> <span> <span>' + output.post[i].time + ' • ' +  output.post[i].location + ' </span>  </span> </div> '+check_if_owner+' </div><div class="post_raw_body" ondblclick="double_click_love(' + output.post[i].id + ')"> <p class="content select_text"> ' + output.post[i].body_text.replace("\n\n","<br>") + ' </p> ' +
                            sa + slidrr + sb +
